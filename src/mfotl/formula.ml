...
let rec preds x = match x with
  ...
  | Conj (f1, f2) | Disj (f1, f2)
    | Imp (f1, f2) | Iff (f1, f2)
    | Until (_, f1, f2) | Since (_, f1, f2) -> let a1s = List.fold_left (fun acc a ->
                                                             if List.mem a acc then acc
                                                             else acc @ [a]) [] (preds f1) in
                                               let a2s = List.fold_left (fun acc a ->
                                                             if (List.mem a acc) || (List.mem a a1s) then acc
                                                             else acc @ [a]) [] (preds f2) in
                                               List.append a1s a2s

let rec ts_to_string str_of ts =
  match ts with
  | [] -> ""
  | (Var x)::ts -> if ts = [] then x
                   else x ^ ", " ^ (ts_to_string str_of ts)
  | (Const d)::ts -> if ts = [] then (str_of d)
                     else (str_of d) ^ ", " ^ (ts_to_string str_of ts)

let rec f_to_string str_of l f = match f with
  | TT -> Printf.sprintf "⊤"
  | FF -> Printf.sprintf "⊥"
  | Pred (r, ts) -> Printf.sprintf "%s(%s)" r (ts_to_string str_of ts)
...
let rec f_to_json str_of indent pos f =
  ...
  | FF -> Printf.sprintf "%s\"%sformula\": {\n%s\"type\": \"FF\"\n%s}"
            indent pos indent' indent
  | Pred (r, ts) -> Printf.sprintf "%s\"%sformula\": {\n%s\"type\": \"Pred\",\n%s\"name\": \"%s\",\n%s\"terms\": \"%s\"\n%s}"
                      indent pos indent' indent' r indent' (ts_to_string str_of ts) indent
  ...
  | Until (i, f, g) -> Printf.sprintf "%s\"%sformula\": {\n%s\"type\": \"Until\",\n%s\"interval\": \"%s\",\n%s,\n%s\n%s}"
                         indent pos indent' indent' (interval_to_string i) (f_to_json str_of indent' "l" f) (f_to_json str_of indent' "r" g) indent
  | _ -> ""
let formula_to_json str_of = f_to_json str_of "    " ""
