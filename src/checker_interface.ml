...
module Checker_interface = struct
  ...
  module E = Expl
  module Expl = Expl.Make (Expl.Proof)
  module Vis = Vis.Expl
  ...
  let check trace_lst f expls =
    ...
    let trace_lst' = convert_trace_aux trace_lst in
  ...
  let false_paths trace_lst f expls =
    ...
    List.rev(List.fold_left expls ~init:[] ~f:(fun acc expl ->
                 ...
                 match paths with
                 | None -> None::acc
                 | Some ps -> Some(List.map (of_poly_set ps) ~f:(fun l ->
                                       List.map l ~f:(fun l' -> of_fset l')))::acc))
