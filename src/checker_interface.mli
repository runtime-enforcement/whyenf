...
val check: (timestamp * (string * Dom.t list, 'a) Base.Set.t) list -> Formula.t -> Expl.Proof.t Expl.Pdt.t list ->
           (bool * (event_data, (event_data sproof, event_data vproof) sum) pdt * Checker_trace.t) list

val false_paths: (timestamp * (string * Dom.t list, 'a) Base.Set.t) list -> Formula.t -> Expl.Proof.t Expl.Pdt.t list ->
